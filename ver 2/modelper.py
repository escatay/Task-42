    ...
    """
        ...
        self.fig.add_trace(
            go.Scatter(
                ...
                y = regression.sin_f(self.data, deg = 1, ks =1),
                name = 'degree 1, ferquency = 1 -> RMSE: ' +"%.2f" % self.mse(yhat = regression.sin_f(self.data, deg = 1, ks =1), y = Y),
                visible = True
            )
        )
        #periodical
        self.fig.add_trace(
            go.Scatter(
                x = np.linspace(0,10,100),
                y = regression.sin_f(self.data, deg = 2, ks = 1),
                name = 'degree 2, ferquency = 1  -> RMSE: ' +"%.2f" % self.mse(yhat = regression.sin_f(self.data, deg = 2, ks=1), y = Y),
                visible = True
            )
        )
        #gausian
        self.fig.add_trace(
            go.Scatter(
                 x = np.linspace(0,10,100),
                y = regression.sin_f(self.data, deg = 2, ks = 2),
                name = 'degree 2, , ferquency = 3 -> RMSE: '+"%.2f" % self.mse(yhat = regression.sin_f(self.data, deg = 2, ks=2), y = Y),
                visible = True
            )
        )
        ...
        self.fig.add_trace(
            go.Scatter(
                 x = np.linspace(0,10,100),
                y = regression.sin_f(self.data, deg = 1, ks = 2),
                name = 'degree 1, frequency = 2  -> RMSE: ' +"%.2f" % self.mse(yhat = regression.sin_f(self.data, deg = 1, ks = 2), y = Y),
                visible = True
            )
        )
