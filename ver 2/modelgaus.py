    ...
    """
        ...
        self.fig.add_trace(
            go.Scatter(
                ...
                name = 'degree 3 -> RMSE: ' +"%.2f" % self.mse(yhat = regression.gaus(self.data, deg = 3), y = Y),
                visible = True
            )
        )
        #periodical
        self.fig.add_trace(
            go.Scatter(
                x = np.linspace(0,10,100),
                y = regression.gaus(self.data, deg = 4),
                name = 'degree 4 -> RMSE: ' +"%.2f" % self.mse(yhat = regression.gaus(self.data, deg = 4), y = Y),
                visible = True
            )
        )
        #gausian
        self.fig.add_trace(
            go.Scatter(
                 x = np.linspace(0,10,100),
                y = regression.gaus(self.data, deg = 5),
                name = 'degree 5 -> RMSE: '+"%.2f" % self.mse(yhat = regression.gaus(self.data, deg = 5), y = Y),
                visible = True
            )
        )
        ...
        self.fig.add_trace(
            go.Scatter(
                 x = np.linspace(0,10,100),
                y = regression.gaus(self.data, deg = 6),
                name = 'degree 6  -> RMSE: ' +"%.2f" % self.mse(yhat = regression.gaus(self.data, deg = 6), y = Y),
                visible = True
            )
        )
